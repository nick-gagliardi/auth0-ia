/**
 * API Route: /api/analytics/feedback
 *
 * Fetches feedback data from Mintlify Analytics API
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pagePath = searchParams.get('page') || undefined;
    ...
    let feedback;
    if (pagePath) {
      feedback = await client.getFeedbackForPage(pagePath, {
        dateFrom,
        dateTo,
        source,
        status,
      ...
      const allFeedback = await client.getAllFeedback({
        dateFrom,
        dateTo,
        source,
        status,
    ...
    console.error('Error fetching Mintlify feedback:', error);

...
function preview(code: string, max = 140): string {
...
export async function extractFencedBlocks(args: {
  ...
  while (i < lines.length) {
    ...
    if (j >= lines.length) break; // unclosed fence
...
export async function buildSnippetMigrationIndex(pages: Array<{ filePath: string; mdx: string }>): Promise<SnippetMigrationIndex> {
  ...
  // group by hash
  const groups = new Map<string, SnippetOccurrence[]>();
  ...
  const items: SnippetMigrationItem[] = [];

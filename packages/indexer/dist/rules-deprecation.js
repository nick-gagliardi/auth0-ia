...
function isFalsePositivePath(filePath) {
...
function deriveSeverity(categories) {
    if (categories.includes('has_rules_code'))
        return 'critical';
    if (categories.includes('is_rules_api') || categories.includes('links_to_rules'))
        return 'high';
    if (categories.includes('suggests_rules') || categories.includes('is_hooks_page'))
        return 'medium';
    return 'low';
}
// ---------------------------------------------------------------------------
// Main export
// ---------------------------------------------------------------------------
export function buildRulesDeprecationIndex(pages) {
    const items = [];
    const byCategory = {
        has_rules_code: 0,
        links_to_rules: 0,
    ...
    for (const page of pages) {
        ...
        if (isFalsePositivePath(page.filePath))
            continue;
        ...
        if (categories.length === 1 && categories[0] === 'mentions_rules' && allEvidence.length <= 2) {
            ...
            const hasAuth0Context = lowerMdx.includes('/docs/customize/rules') ||

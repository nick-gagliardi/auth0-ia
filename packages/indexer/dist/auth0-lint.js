...
function seemsLikeOauthFlow(text) {
    return hasAny(text, [
...
export function lintAuth0MdxPages(pages) {
    const items = [];
    ...
    for (const p of pages) {
            ...
            if (/authorization\s+code/.test(text) || /response_type\s*=\s*code/.test(text) || /\/authorize\b/.test(text)) {
                if (!mentionsPkce(text)) {
                    ...
                    warnings.push({
                        ...
                        severity: 'warn',
                        message: 'OAuth Authorization Code flow appears in this page, but PKCE is not mentioned (PKCE is recommended for public clients).',
        ...
        if (mentionsLocalStorage(text) && !hasStorageWarningLanguage(text)) {
            ...
            warnings.push({
                ...
                severity: 'error',
                message: 'This page mentions localStorage. Storing tokens in localStorage is a common XSS footgun; ensure the guidance is explicit and safe.',
                evidence: hit?.evidence,
                line: hit?.line
            });
        }
        if (mentionsSessionStorage(text) && !hasStorageWarningLanguage(text)) {
            ...
            warnings.push({
                ...
                severity: 'warn',
                message: 'This page mentions sessionStorage. Storing tokens in Web Storage can be an XSS footgun; ensure the guidance is explicit and safe.',
                evidence: hit?.evidence,
                line: hit?.line
            });
        }
        if (mentionsCookies(text) &&
            ...
            warnings.push({
                ...
                severity: 'info',
                message: 'This page mentions cookies alongside tokens. Consider explicitly noting secure cookie attributes (HttpOnly/Secure/SameSite) where appropriate.',
                evidence: hit?.evidence,
                line: hit?.line
            });
        }
        ...
        if (mentionsRulesOrHooks(text) && !mentionsActions(text)) {
            ...
            warnings.push({
                ...
                severity: 'warn',
                message: 'This page references Auth0 Rules/Hooks without mentioning Actions. Rules/Hooks are legacy; consider updating wording to Actions or explicitly marking legacy guidance.',
                evidence: hit?.evidence,
                line: hit?.line
            });
        }
